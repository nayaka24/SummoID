									...
									<div class="header-icons">
										...
										<li><a href="{{url ('/profiluser')}}" aria-haspopup="true" aria-expanded="false" v-pre><span class="fas fa-user-alt"></span> {{ Auth::user()->name }}</a>
											<ul class="sub-menu">
												...
												<li><a class="" href="{{ url('/checkout') }}"><i class="fas fa-cash-register"></i></a> Checkout</li>
						...
						<table class="cart-table">
							<thead class="cart-table-head">
								<tr class="table-head-row">
									<th class="product-name">Tanggal Pemesanan</th>
									<th class="product-name">Jenis Pesanan</th>
									<th class="product-name">Alamat</th>
									<th class="product-name">Catataan</th>
							...
							@foreach($Transaksi as $result =>$hasil)
								@if($hasil->carts->users_id == Auth::user()->id_user)
								<tr class="table-body-row">
									<td class="product-name">{{$hasil->tanggal_ambil}} - {{$hasil->tanggal_kembali}}</td>
									@if($hasil->jenis_transaksi == '1')
                                        <td class="product-name">Pesanan Kendaraan</td>
                                    @else
                                        <td class="product-name">Pesanan Penginapan</td>
                                    @endif
                                    <td class="product-name">{{$hasil->alamat}}</td>
                                    <td class="product-name">{{$hasil->catatan}}</td>
									<td class="product-price">RP.{{$hasil->total}}</td>
									@if($hasil->pembayaran == '1')
                                        <td class="product-name">Pesanan belum dibayar</td>
                                    @else
                                        <td class="product-name">Pesanan Sudah dibayar</td>
                                    @endif
								</tr>
								@endif
							@endforeach
